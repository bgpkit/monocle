...
/// find and collect data file bins, each bin contains a group of files that should be processed together.
pub fn find_bins(ts_start: f64, ts_end: f64, bin_width: i64) -> Vec<DataFileBin> {
    ...
    for (key, group) in &items.into_iter().group_by(|item| item.ts_start.timestamp() / bin_width) {
